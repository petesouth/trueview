 ...
 */
monitcommandoMainModule.globalAppData.controllers.serviceFullPieChartViewController = function(
		...
		var settings = {
			...
			drawBefore : function(renderer, rect) {
				sz = renderer.measureText($scope.extradata.dataOrb.valueText,
						0, {
							'class' : 'chart-inner-text-sparkline'
						});
				renderer.text($scope.extradata.dataOrb.valueText, rect.x
						+ (rect.width - sz.width) / 2, rect.y - 5
						+ (rect.height / 2), 0, 0, 0, {
					'class' : 'chart-inner-text-sparkline'
				});
			},
