 ...
 */
angular.module('tv')
.directive('sideMenuItem', function($state,$rootScope) {
    return {
        restrict: 'E',
        require: '^sideMenu',
        scope: {
        ...
        controller: function($scope) {
            ...
            $scope.$watch(function() {
                ...
                $scope.alert_count = $rootScope.new_msg;
            }, true);

        },
        link: function($scope) {
            // load a template for this directive based on the type ( link | dropdown )
            $scope.itemTemplate = 'trueview/components/side-menu/side-menu-' + $scope.item.type + '.tmpl.html';
            ...
            function isActive() {
                ...
                var currentURL = "#"+$state.current.url
                    var menu_changed = currentURL.includes($scope.item.url);
                return menu_changed;
            }
